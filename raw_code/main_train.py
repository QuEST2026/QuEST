...
if __name__ == '__main__':
    ...
    dataset_size = len(train_poisoned_dataloader)
    ...
    for epoch in range(250):
        ...
        for i, data in enumerate(train_poisoned_dataloader):
            ...
            if model.total_steps % opt.loss_freq == 0:
                ...
                with open(log_path, 'a') as f:
                    text = '\n' + str(model.loss_record[0]) + " " + str(model.loss_record[1]) + " " + str(model.loss_record[2]) + " " + str(model.loss_record[3])
                    f.write(text)

        if (epoch % 10 == 0):
            logger.info('saving the latest model %s (epoch %d, model.total_steps %d)' %
                        ("cifar", epoch, model.total_steps))
        ...
        if opt.is_QBATrain:
            acc, target_acc = validate_cls(model, val_opt)
